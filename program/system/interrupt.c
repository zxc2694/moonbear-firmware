	...
	if (TIM_GetITStatus(TIM2, TIM_IT_CC1) == SET) {
		...
		if (pwm3_is_rising) {
			...
			if (current[0] > pwm3_previous_value)
				global_var[PWM3_CCR].value =  current[0] - pwm3_previous_value;
			else if (current[0] < pwm3_previous_value)
				global_var[PWM3_CCR].value = 0xFFFF - pwm3_previous_value + current[0] ;
	...
	if (TIM_GetITStatus(TIM2, TIM_IT_CC2) == SET) {
		...
		if (pwm4_is_rising) {
			...
			if (current[1] > pwm4_previous_value)
				global_var[PWM4_CCR].value =  current[1] - pwm4_previous_value;
			else if (current[1] < pwm4_previous_value)
				global_var[PWM4_CCR].value = 0xFFFF - pwm4_previous_value + current[1] ;
	...
	if (TIM_GetITStatus(TIM2, TIM_IT_CC3) == SET) {
		...
		if (pwm5_is_rising) {
			...
			if (current[2] > pwm5_previous_value)
				global_var[PWM5_CCR].value =  current[2] - pwm5_previous_value;
			else if (current[2] < pwm5_previous_value)
				global_var[PWM5_CCR].value = 0xFFFF - pwm5_previous_value + current[2] ;
	...
	if (TIM_GetITStatus(TIM4, TIM_IT_CC1) == SET) {
		...
		if (pwm1_is_rising) {
			...
			if (current[0] > pwm1_previous_value)
				global_var[PWM1_CCR].value =  current[0] - pwm1_previous_value;
			else if (current[0] < pwm1_previous_value)
				global_var[PWM1_CCR].value = 0xFFFF - pwm1_previous_value + current[0] ;
	...
	if (TIM_GetITStatus(TIM4, TIM_IT_CC2) == SET) {
		...
		if (pwm2_is_rising) {
			...
			if (current[1] > pwm2_previous_value)
				global_var[PWM2_CCR].value =  current[1] - pwm2_previous_value;
			else if (current[1] < pwm2_previous_value)
				global_var[PWM2_CCR].value = 0xFFFF - pwm2_previous_value + current[1] ;
