...
void Update_RC_Control(int16_t *Roll, int16_t  *Pitch, int16_t  *Yaw, int16_t  *Thr, uint8_t *safety)
{
	...
	if ((global_var[PWM3_CCR].value > MIN_PWM_INPUT) &&
	    (global_var[PWM3_CCR].value < MAX_PWM_INPUT)) {

		*Thr = (PWM_MOTOR_MAX - PWM_MOTOR_MIN) / (MAX_PWM3_INPUT - MIN_PWM3_INPUT) *
		       (global_var[PWM3_CCR].value - MIN_PWM3_INPUT) + PWM_MOTOR_MIN;

	}

	/*Get PWM1 Input capture to control roll*/
	if ((global_var[PWM1_CCR].value > MIN_PWM_INPUT) &&
	    (global_var[PWM1_CCR].value < MAX_PWM_INPUT)) {

		//*Roll = (MAX_CTRL_ROLL - MIN_CTRL_ROLL) / (MAX_PWM1_INPUT - MIN_PWM1_INPUT) *
		//        (global_var[PWM1_CCR].value - MIN_PWM1_INPUT) + MIN_CTRL_ROLL;

		*Roll = MAX_CTRL_ROLL / (MAX_PWM1_INPUT - MID_PWM1_INPUT) *
			(global_var[PWM1_CCR].value - MID_PWM1_INPUT) ;


	}

	/*Get PWM2 Input capture to control pitch*/
	if ((global_var[PWM2_CCR].value > MIN_PWM_INPUT) &&
	    (global_var[PWM2_CCR].value < MAX_PWM_INPUT)) {

		//*Pitch = (MAX_CTRL_PITCH - MIN_CTRL_PITCH) / (MAX_PWM2_INPUT - MIN_PWM2_INPUT) *
		//        (global_var[PWM2_CCR].value - MIN_PWM2_INPUT) + MIN_CTRL_PITCH;

		*Pitch = MAX_CTRL_PITCH / (MAX_PWM2_INPUT - MID_PWM2_INPUT) *
			 (global_var[PWM2_CCR].value - MID_PWM2_INPUT) ;

	}

	/*Get PWM4 Input capture to control yaw*/
	if ((global_var[PWM4_CCR].value > MIN_PWM_INPUT) &&
	    (global_var[PWM4_CCR].value < MAX_PWM_INPUT)) {

		//*Yaw = (MAX_CTRL_YAW- MIN_CTRL_YAW) / (MAX_PWM4_INPUT - MIN_PWM4_INPUT) *
		//        (global_var[PWM4_CCR].value - MIN_PWM4_INPUT) + MIN_CTRL_YAW;

		*Yaw = MAX_CTRL_YAW / (MAX_PWM4_INPUT - MID_PWM4_INPUT) *
		       (global_var[PWM4_CCR].value - MID_PWM4_INPUT) ;
