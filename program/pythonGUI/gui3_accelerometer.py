...
# set plot to animated
		...
		self.v1line, = plt.plot(analogData.v1,label="Accelerometer_X",color="red")
		self.v2line, = plt.plot(analogData.v2,label="Accelerometer_Y",color="orange")
		self.v3line, = plt.plot(analogData.v3,label="Accelerometer_Z",color="green")


		plt.xlabel("Time")
		plt.ylabel("Acceleration of gravity")
		plt.title("Measure Acceleration")
		plt.legend()		#Show label figure.
		plt.ylim([-0.5, 1.5]) # Vertical axis scale.
...
# expects 1 arg - serial port string
	if(len(sys.argv) != 2):
		...
		print "python gui3_accelerometer.py '/dev/ttyUSB0'"
		exit(1)
